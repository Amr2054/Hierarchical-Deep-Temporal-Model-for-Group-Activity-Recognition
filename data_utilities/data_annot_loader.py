    ...
    with open(path, 'r') as file:
        ...
        for idx,line in enumerate(file):
            ...
            if box_info.player_ID > 11:
                continue
            player_boxes[box_info.player_ID].append(box_info) # list of box info objects one box for each frame

        for idx, boxes_info in player_boxes.items():
            # we take 9 frames only out of the 20 frames for each player
...
def load_volleyball_dataset(videos_root, annot_root):
    ...
    for idx, video_dir in enumerate(videos_dirs):
        ...
        for clip_dir in clips_dir:
            ...
            frame_boxes_dct = create_frame_boxes_dct(annot_file)
